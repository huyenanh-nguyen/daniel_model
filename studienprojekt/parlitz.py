...
def parlitz(par : list, t : list, alpha, omega, delta):
    """Duffing Oscillator will get driven by the a * cos(omega *t)
    The coupling is linear and onesided.
        x -> y
        y -> -x - x**3 - delta * y + alpha * np.cos(2 * np.pi * z)
        z -> omega / 2 * np.pi
    ...
    Args:
        ...
        omega (float): non linear restoring force
        delta (float): 
    ...
    x,y,z = par
    dx = y
    dy = -x - x**3 - (delta * y) + (alpha * np.cos(2 * np.pi * z))
    dz = omega / 2 * np.pi
...
class Parlitz:
    ...
    def __init__(self, par : list, t : list, t_keep : int,  alpha, omega, delta):
        """_summary_
    ...
    def duffvdpsolver(self):
        ...
        Returns:
           List: index 0 -> x, Index 1 -> y, Index 2 -> z
        """
        par = self.par
        t = self.t
        omega = self.omega
        ...
        sol = odeint(parlitz, par, t, args = (alpha, omega, delta), rtol= 0.000001)
    ...
    def duffvdpsolver_tolerance(self, rtol, atol):
        ...
        sol = odeint(parlitz, par, t, args = (alpha, omega, delta), rtol= rtol, atol= atol)
        ...
        Returns:
            List: two Indexes with n length of arrays -> depending on how many Peaks i have 
                  ...
                  >>> [(t[i-1],t[i],t[i+1]), ...][(sol[i-1,u],sol[i, u],sol[i+1, u]), ...]
                      Index 0 -> time (and each for x,y,z)
                      Index 1 -> Solution (and each for x,y,z)
        """
        t = self.t
        sol = self.duffvdpsolver()
        keep = self.t_keep
